...
function my_outputArticles()
{
    if (isset($_COOKIE['Username'])) {
        ...
        if ($totalCount > 0) {
            $sql = "select travelimage.ImageID, path, content, description, title from travelimagefavor, travelimage 
where travelimage.imageid=travelimagefavor.imageid and travelimagefavor.uid = $uid LIMIT $mark, $pageSize";
            ...
            while ($row = $statement->fetch()) {
                ...
                $title = $row['title'];
                my_outputArticlesTag($iid, $username, $path, $description, $title);

                echo "";
            }
        } else echo "<div class=\"col-xs-12 col-sm-12 text-center\"><h4>还没有收藏图片</h4></div>";
...
function my_outputArticlesTag($iid, $username, $path, $description, $title)
{
